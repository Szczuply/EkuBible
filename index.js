...
SelectBook.addEventListener("change", ()=>{refreshChapters(SelectBook.value)});
SelectChapter.addEventListener('change', ()=>{getVerses(SelectBook.value, SelectChapter.value)});
nextChapterButton.addEventListener('click', ()=>{getVerses(SelectBook.value, (currentChapter+1))})
prevChapterButton.addEventListener('click', ()=>{getVerses(SelectBook.value, (currentChapter-1))})
...
function getVerses(BookNumber, ChapterNumber){
  if((ChapterNumber<0) || (ChapterNumber >= BWBible.BibliaEkumenicznaKsiegi[BookNumber].ROZDZIALY.length )){
    alert('Przepraszamy, ale nie znaleźliśmy podanego rozdziału')
  }
  else{
    document.querySelector('.content').innerHTML = ''
    currentChapter = new Number(ChapterNumber);
    BWBible.BibliaEkumenicznaKsiegi[BookNumber].ROZDZIALY[ChapterNumber].WERSETY.forEach((x, index) =>{
